//! Kimi batch processing implementation.
...
impl BatchProviderClient for Client {
    async fn create_batch(&self, requests: Vec<BatchRequest>) -> Result<BatchJob, RouterError> {
        ...
        for req in &requests {
            let mut kimi_req = self.transformer.transform_request(&req.request);
            kimi_req.stream = Some(false);

            let body = serde_json::to_value(&kimi_req)
        ...
        let resp = self.http
            ...
            .send()
            .await
            .map_err(|e| err_provider_unavailable(Provider::Kimi, format!("request failed: {}", e)))?;
        ...
        let batch: BatchObject = resp.json().await
            .map_err(|e| err_server_error(Provider::Kimi, format!("decode response: {}", e)))?;
    ...
    async fn get_batch_results(&self, batch_id: &str) -> Result<Vec<BatchResult>, RouterError> {
        ...
        let output_file_id = job.metadata.get("output_file_id")
            ...
            .map(|s| s.to_string())
            .filter(|s| !s.is_empty())
            .ok_or_else(|| err_invalid_request("batch has no output file").with_provider(Provider::Kimi))?;
        ...
        let content = resp.text().await
            .map_err(|e| err_server_error(Provider::Kimi, format!("read response: {}", e)))?;
            ...
            if let Ok(output) = serde_json::from_str::<BatchOutputLine>(line) {
                let result = if let Some(err) = output.error {
                    BatchResult {
                        ...
                        request_labels: None,
                        response: None,
                        error: Some(err_server_error(Provider::Kimi, err.message)),
                    }
...
impl Client {
    async fn upload_batch_file(&self, content: Vec<u8>) -> Result<String, RouterError> {
        ...
        let resp = self.http
            ...
            .send()
            .await
            .map_err(|e| err_provider_unavailable(Provider::Kimi, format!("upload failed: {}", e)))?;
        ...
        let file_resp: FileUploadResponse = resp.json().await
            .map_err(|e| err_server_error(Provider::Kimi, format!("decode upload response: {}", e)))?;
