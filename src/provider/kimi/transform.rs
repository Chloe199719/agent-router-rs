//! Kimi request/response transformation.
...
impl Transformer {
    ...
    /// Transform a unified request to Kimi format.
    pub fn transform_request(&self, req: &CompletionRequest) -> ChatCompletionRequest {
        let mut kimi_req = ChatCompletionRequest {
        ...
        if !req.tools.is_empty() {
            let kimi_tools = self.schema_translator.tools_to_openai(&req.tools);
            kimi_req.tools = Some(
                kimi_tools
                    .into_iter()
                    .map(|t| Tool {
        ...
        if let Some(ref tc) = req.tool_choice {
            kimi_req.tool_choice = Some(self.transform_tool_choice(tc));
        }
    ...
    /// Transform Kimi response to unified format.
...
mod tests {
    ...
    fn transform_includes_metadata_in_json() {
        ...
        .with_metadata(m);
        let kimi = t.transform_request(&req);
        let v = serde_json::to_value(&kimi).unwrap();
