...
impl Client {
    ...
    pub async fn handle_error_response(&self, resp: reqwest::Response) -> RouterError {
        ...
        err_server_error(Provider::Kimi, body).with_status_code(status)
    }

    fn map_api_error(&self, api_err: &APIError, status_code: u16) -> RouterError {
        match status_code {
            401 => err_invalid_api_key(Provider::Kimi).with_status_code(status_code),
            429 => err_rate_limit(Provider::Kimi, &api_err.message).with_status_code(status_code),
            404 => err_model_not_found(Provider::Kimi, &api_err.message).with_status_code(status_code),
            400 => {
                if api_err.message.contains("context_length") {
                    err_context_length(Provider::Kimi, &api_err.message).with_status_code(status_code)
                } else {
            ...
            _ => err_server_error(Provider::Kimi, &api_err.message).with_status_code(status_code),
        }
    }
}
...
impl ProviderClient for Client {
    ...
    async fn models(&self) -> Result<Vec<String>, RouterError> {
        ...
        let resp = builder.send().await.map_err(|e| {
            err_provider_unavailable(Provider::Kimi, format!("request failed: {}", e))
        })?;
        ...
        let body: ListModelsResponse = resp.json().await.map_err(|e| {
            err_server_error(Provider::Kimi, format!("failed to decode models list: {}", e))
        })?;
    ...
    async fn complete(&self, req: &CompletionRequest) -> Result<CompletionResponse, RouterError> {
        let mut kimi_req = self.transformer.transform_request(req);
        kimi_req.stream = Some(false);
        ...
        let kimi_resp: ChatCompletionResponse = resp.json().await.map_err(|e| {
            err_server_error(Provider::Kimi, format!("failed to decode response: {}", e))
        })?;

        self.transformer.transform_response(&kimi_resp)
            .ok_or_else(|| err_server_error(Provider::Kimi, "empty response"))
    }

    async fn stream(&self, req: &CompletionRequest) -> Result<StreamResponse, RouterError> {
        let mut kimi_req = self.transformer.transform_request(req);
        kimi_req.stream = Some(true);
        kimi_req.stream_options = Some(StreamOptions { include_usage: true });
        ...
        let event_stream = kimi_sse_stream(byte_stream, transformer);

        Ok(Box::pin(event_stream))
    }
}

/// Parse Kimi SSE stream into StreamEvents.
fn kimi_sse_stream(
    ...
    async_stream::stream! {
        ...
        while let Some(chunk_result) = stream.next().await {
            let chunk = match chunk_result {
                Ok(c) => c,
                Err(e) => {
                    yield Err(err_provider_unavailable(Provider::Kimi, format!("stream error: {}", e)));
                    return;
                }
            };
